<template>
  <section class="grid grid-cols-1 md:grid-cols-12 gap-8 px-8">
    ...
    <div class="md:col-span-4 grid grid-cols-2 gap-4">
      <div v-for="(src, i) in props.images?.slice(1, 5)" :key="i" class="rounded-xl overflow-hidden shadow bg-gray-100">
        <div class="aspect-[4/3] h-full">
          <img :src="fileUrl(src.filePath, 'car')" :alt="`Car detail ${i + 1}`" class="w-full h-full object-cover"
            loading="lazy" />
        </div>
      </div>
    </div>
    ...
    <transition name="fade">
      <div v-if="openAll" class="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex" @click.self="openAll = false">
        <div class="m-auto w-[95vw] max-w-6xl bg-white rounded-2xl p-4 md:p-6">
          <div class="flex items-center justify-between mb-4">
            ...
            <button class="p-2 rounded hover:bg-gray-100" @click="openAll = false" aria-label="Cerrar">
          ...
          <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 md:gap-4">
            <a v-for="(src, i) in props.images" :key="`all-${i}`" href="#" target="_blank" rel="noopener"
              class="block rounded-lg overflow-hidden bg-gray-100">
              <div class="aspect-[4/3]">
                <img :src="fileUrl(src.filePath, 'car')" :alt="`Foto ${i + 1}`" class="w-full h-full object-cover"
                  loading="lazy" />
              </div>
            </a>
          </div>
...
import type { MediaLite } from "~/types/car";

type Props = { images: MediaLite[] };
