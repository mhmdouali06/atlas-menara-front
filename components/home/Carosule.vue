...
<template>
  <section class="mt-24 px-4 md:px-12">
    <div class="flex flex-col md:flex-row items-center justify-between">
      <div>
        <p class="font-volkhov text-orange text-xl mb-2 md:mb-0 ">Explora</p>
        <h3 class="font-poppins text-2xl md:text-3xl mb-8 font-semibold">
        ...
        <div class="flex gap-4 overflow-x-auto">
          <button v-for="category in categoriesList" :key="category.id" @click="activeCategory = category.slug" :class="[
            activeCategory === category.slug
              ...
              : 'bg-white text-[#08133E] border-[#08133E]',
            'category-button text-nowrap rounded-full px-5 py-2 text-lg font-semibold border  hover:bg-orange hover:border-orange hover:text-white transition duration-300 easy',
          ]">
            {{ category.name }}
          </button>
        </div>
    ...
    <div>
      <Carousel v-if="!loading" breakpointMode="carousel" ref="swiperRef" :breakpoints="{
        ...
        768: { itemsToShow: 3 },
        1024: { itemsToShow: 4 },
      }" :wrap-around="false" snap-align="start" :gap="20">
    ...
    <div class="flex justify-end gap-4 mt-8 mb-8 md:mb-0">
...
button:not(.category-button):hover {
  background: #08133E;
  box-shadow: 0 1px 9.1px 4px #08133E;
  border-color: #07123d;
}

button:not(.category-button):hover img {
