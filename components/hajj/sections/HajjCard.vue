<template>
  <article class="hajj-card relative rounded-lg  bg-white shadow-lg overflow-hidden w-full">
    <div class="grid grid-cols-12  items-center">
      <!-- Left image -->
      <div class="col-span-12 md:col-span-5  h-full ">
        <div class="relative  overflow-hidden h-44 sm:h-48 md:h-full ">
          <img :src="fileUrl(item?.thumbnail?.filePath, 'hajj')" :alt="item.title"
            class="w-full h-full object-fill sm:rounded-[12px] md:rounded-none md:rounded-s-[12px]" />
          <div class="absolute top-3 right-3 bg text-xs px-2 py-2 rounded-md font-semibold">
            {{ props.item.gallery.length }} imágenes
          </div>
        </div>
      </div>

      <!-- Right content -->
      <div class="col-span-12 md:col-span-7  p-4  box-content ">
        <div>
          <div class="flex flex-col md:flex-row justify-between items-start">
            <div>
              <h3 class="text-[#0E2041] font-poppins text-base sm:text-lg md:text-xl font-extrabold leading-tight">
                {{ item.title }}
                <sub class="text-[12px] mt-1 font-poppins text[#112211] text-opacity-75">
                  ( {{ formatDuration(props.item.startsAt, props.item.endsAt) }} )
                </sub>
              </h3>
              <p class="text-sm text-[#112211] text-opacity-75 mt-1">
                Inicio: {{ formatDateTextEs(props.item.startsAt) }}<br />
                Fin: {{ formatDateTextEs(props.item.endsAt) }}
              </p>
            </div>

            <div class="text-left mt-4 md:mt-1 md:text-right font-poppins">
              <p class="text-xs md:text-sm text-[#112211] text-opacity-75">Paquete estándar</p>
              <p class="text-xl md:text-2xl font-extrabold text-orange">
                {{ item.price }} €
              </p>
              <p class="text-[10px] md:text-xs text-[#112211] text-opacity-75 text-nowrap">
                Precio real aproximado : {{ item.premiumPrice }} €
              </p>
            </div>
          </div>

          <ul class="mt-3 text-sm text-gray-600 space-y-1">
            <ul class="flex flex-wrap gap-3 items-center">
              <li class="flex items-center gap-2 ">
                <div class="flex gap-1">
                  <svg class="w-4 h-4 text-[#FF8682] " v-for="value in item.averageRating" :key="value"
                    viewBox="0 0 15 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path
                      d="M11.8112 14.0005C11.7059 14.0009 11.6033 13.9681 11.5177 13.9067L7.49869 10.993L3.47962 13.9067C3.39376 13.969 3.29031 14.0024 3.18424 14.002C3.07817 14.0016 2.97497 13.9675 2.88956 13.9046C2.80415 13.8417 2.74096 13.7532 2.70912 13.6521C2.67728 13.5509 2.67844 13.4422 2.71244 13.3417L4.27994 8.69891L0.217436 5.91298C0.12944 5.8527 0.063025 5.76586 0.0278959 5.66515C-0.00723315 5.56444 -0.00923555 5.45513 0.0221813 5.3532C0.0535982 5.25127 0.116788 5.16206 0.202518 5.0986C0.288247 5.03514 0.392024 5.00076 0.498686 5.00048H5.51056L7.02306 0.34579C7.05564 0.245296 7.11922 0.157706 7.20467 0.0955837C7.29011 0.0334614 7.39304 0 7.49869 0C7.60433 0 7.70726 0.0334614 7.79271 0.0955837C7.87815 0.157706 7.94173 0.245296 7.97431 0.34579L9.48681 5.00204H14.4987C14.6055 5.00199 14.7095 5.03613 14.7955 5.09947C14.8815 5.16281 14.9449 5.25201 14.9765 5.35402C15.0081 5.45603 15.0063 5.56548 14.9712 5.66634C14.9361 5.76721 14.8696 5.85419 14.7815 5.91454L10.7174 8.69891L12.284 13.3405C12.3094 13.4156 12.3165 13.4957 12.3048 13.5742C12.2931 13.6526 12.2629 13.7272 12.2168 13.7917C12.1706 13.8561 12.1097 13.9087 12.0392 13.945C11.9687 13.9813 11.8905 14.0003 11.8112 14.0005Z"
                      fill="#FF8682" />
                  </svg>


                </div>
                <span>Hotel {{ item.averageRating }} estrellas</span>
              </li>

              <li class="flex items-center gap-2" v-if="item.includesTransport">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                  <path
                    d="M13.5 2H3C2.86739 2 2.74021 2.05268 2.64645 2.14645C2.55268 2.24021 2.5 2.36739 2.5 2.5V8.5C2.50091 9.29537 2.81727 10.0579 3.37968 10.6203C3.9421 11.1827 4.70463 11.4991 5.5 11.5H9C9.79537 11.4991 10.5579 11.1827 11.1203 10.6203C11.6827 10.0579 11.9991 9.29537 12 8.5V6H12.5625C13.0762 5.99942 13.5687 5.79511 13.9319 5.43188C14.2951 5.06865 14.4994 4.57618 14.5 4.0625V3C14.5 2.73478 14.3946 2.48043 14.2071 2.29289C14.0196 2.10536 13.7652 2 13.5 2ZM13.5 4.0625C13.5 4.31114 13.4012 4.5496 13.2254 4.72541C13.0496 4.90123 12.8111 5 12.5625 5H12V3H13.5V4.0625ZM12.5 12.5H2C1.86739 12.5 1.74021 12.5527 1.64645 12.6464C1.55268 12.7402 1.5 12.8674 1.5 13C1.5 13.1326 1.55268 13.2598 1.64645 13.3536C1.74021 13.4473 1.86739 13.5 2 13.5H12.5C12.6326 13.5 12.7598 13.4473 12.8536 13.3536C12.9473 13.2598 13 13.1326 13 13C13 12.8674 12.9473 12.7402 12.8536 12.6464C12.7598 12.5527 12.6326 12.5 12.5 12.5Z"
                    fill="#112211" />
                </svg>
                <span>Comidas y Traslados</span>
              </li>
            </ul>

            <li class="text-xs text-gray-400 mt-2 flex flex-wrap gap-2 items-center">
              <span class="border border-[#0E2041] p-1 rounded-sm">
                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="15" viewBox="0 0 18 15" fill="none">
                  <path
                    d="M13.5 13.3333H15.3V6.66667H9.9V13.3333H11.7V8.33333H13.5V13.3333ZM0.9 13.3333V0.833333C0.9 0.61232 0.994821 0.400358 1.1636 0.244078C1.33239 0.0877973 1.56131 0 1.8 0H14.4C14.6387 0 14.8676 0.0877973 15.0364 0.244078C15.2052 0.400358 15.3 0.61232 15.3 0.833333V5H17.1V13.3333H18V15H0V13.3333H0.9ZM4.5 6.66667V8.33333H6.3V6.66667H4.5ZM4.5 10V11.6667H6.3V10H4.5ZM4.5 3.33333V5H6.3V3.33333H4.5Z"
                    fill="#0E2041" />
                </svg>
              </span>
              <span class="font-bold text-[#0E2041]">En {{ item.hotels[0].name2 }}:</span>
              {{ item.hotels[0].name }} -
              <span v-if="item.hotels[1]" class="font-bold text-[#0E2041]">En {{ item.hotels[1].name2 }}:</span>
              <span v-if="item.hotels[1]">{{ item.hotels[1].name }}</span>
            </li>
          </ul>
        </div>

        <div class="border-b my-3 opacity-75 border-[#112211]"></div>

        <div class="flex items-center justify-between">
          <button @click="navigateTo(`/hajj/${item.slug}`)"
            class="px-6 py-2.5 w-full rounded-lg bg-[#0E2041] text-white font-semibold shadow hover:opacity-95">
            Ver
          </button>
        </div>
      </div>
    </div>
  </article>
</template>

<script setup lang="ts">
import { formatDateEs, formatDateTextEs, formatDuration } from '~/helpers/functions/formatDuration';
import { fileUrl } from '~/helpers/functions/imageURl';
import type { TravelPackage } from '~/types/travel-package';

const props = defineProps<{ item: TravelPackage }>();
</script>

<style scoped>
.bg {
  border-radius: 8px;
  background: rgba(255, 255, 255, 0.5);
  backdrop-filter: blur(2px);
  color: #112211;
}

.hajj-card {
  transition: all 0.5s ease;
}

.text-orange-500 {
  color: #f59e0b;
}

/* Slightly reduce the active scale on very small screens to avoid overflow */
@media (max-width: 640px) {
  :deep(.swiper-slide-active .hajj-card) {
    transform: scale(1.05);
  }
}
</style>
