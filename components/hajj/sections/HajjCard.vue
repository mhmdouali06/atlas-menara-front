<template>
  <article
    class="hajj-card relative rounded-2xl bg-white shadow-lg overflow-hidden w-full"
  >
    <div class="grid grid-cols-12 gap-4 p-4 md:p-6 items-center">
      ...
      <div class="col-span-12 md:col-span-5">
        <div class="relative rounded-lg overflow-hidden h-40 md:h-48">
          ...
          />
          <div
            class="absolute top-3 right-3 bg text-xs px-2 py-2 rounded-md font-semibold"
          >
            {{ item.imagesCount }} imágenes
          </div>
        </div>
      </div>
      ...
      <div class="col-span-12 md:col-span-7">
        <div>
          <div class="flex justify-between items-start gap-3">
            <div>
              <h3
                class="text-[#0E2041] font-poppins text-lg font-extrabold leading-tight"
              >
                {{ item.title }}
              </h3>
              <p class="text-sm mt-1 font-poppins text[#112211]">
                {{ item.duration }} · Inicio: {{ item.start }}
              </p>
            </div>
          ...
          <ul class="mt-3 text-sm text-gray-600 space-y-1">
            <li class="flex items-center gap-2">
              ...
              <span>{{ item.rating }} · {{ item.hotel }}</span>
            </li>
            <li class="flex items-center gap-2">
              <svg
...
const props = defineProps<{ item: HajjItem }>();
</script>
